...
void AddFirewallRuleCmdline(LPWSTR exeName, LPWSTR exeFile, LPCWSTR dir)
{
    ...
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to change firewall rule : %Id, last error: %d", (INT_PTR)hi, GetLastError());
    }
    else {
...
void RemoveFirewallRuleCmdline(LPWSTR exeName)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to change firewall rule \"%ls\" : %Id, last error: %d", rulename, (INT_PTR)hi, GetLastError());
    }
    else {
...
UINT __stdcall AddRegSoftwareSASGeneration(__in MSIHANDLE hInstall)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to add registry name \"%ls\", %Id, %d", valueName, (INT_PTR)hi, GetLastError());
    }
    else {
    ...
    __in MSIHANDLE hInstall)
{
    ...
    // https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shellexecutew
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to remove amyuni idd : %Id, last error: %d", (INT_PTR)hi, GetLastError());
    }
    else {
...
void TryCreateStartServiceByShell(LPWSTR svcName, LPWSTR svcBinary, LPWSTR szSvcDisplayName)
{
    ...
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to create service with shell : %Id, last error: 0x%02X.", (INT_PTR)hi, GetLastError());
    }
    else {
    ...
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to start service with shell : %Id, last error: 0x%02X.", (INT_PTR)hi, GetLastError());
    }
    else {
...
void TryStopDeleteServiceByShell(LPWSTR svcName)
{
    ...
    hi = ShellExecuteW(NULL, L"open", L"cmd.exe", szCmd, NULL, SW_HIDE);
    if ((INT_PTR)hi <= 32) {
        WcaLog(LOGMSG_STANDARD, "Failed to delete service with shell : %Id, last error: 0x%02X.", (INT_PTR)hi, GetLastError());
    }
    else {
